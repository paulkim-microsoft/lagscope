...
// static void json_formatter(file, latency, frequncy)
...
void json_histogram_dump(int start, int len, int count, unsigned long max_latency, char *json_name)
{
    int err_check = 0;
    int i = 0;
    unsigned long freq_counter = 0;
    ...
    for(lat_intervals = start; lat_intervals < final_interval; lat_intervals+=len)
    {
        ...
        {
            fprintf(json_file, "\n\t\t{")
            fprintf(json_file,"\n\t\t\t\"latency\": %lu,", lat_intervals);
            fprintf(json_file,"\n\t\t\t\"frequency\": %d", 0);
            fprintf(json_file, "\n\t\t},")
            if(lat_intervals + len == final_interval)
            {
                fprintf(json_file, "\n\t\t{")
                fprintf(json_file,"\n\t\t\t\"latency\": %lu,", lat_intervals + len);
                fprintf(json_file,"\n\t\t\t\"frequency\": %d", 0);
                fprintf(json_file, "\n\t\t}")
            }
            continue;
    ...
    if(final_interval < max_latency)
    {
        ...
        fprintf(json_file, "\n\t\t{")
        fprintf(json_file,"\n\t\t\t\"latency\": %lu,", after_final_interval);
        fprintf(json_file,"\n\t\t\t\"frequency\": %lu", freq_counter);
        fprintf(json_file, "\n\t\t}")
    }
