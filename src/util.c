...
{
	printf("Author: %s\n", AUTHOR_NAME);
	printf("lagscope: [-r|-s|-D|-f|-6|-u|-p|-b|-B|-z|-t|-n|-i|-H|-a|-l|-c|-V|-h|-G]\n\n");
	...
	printf("\t-G   [CLIENT ONLY] print percentile(s) of all latencies\n");
...
int parse_arguments(struct lagscope_test *test, int argc, char **argv)
{
	...
	while ((flag = getopt_long(argc, argv, "r::s::Df:6up:b:B:z:t:n:i:Ha:l:c:Vh:Gk:j:", longopts, NULL)) != -1) {
		switch (flag) {
		case 'r':
			test->server_role = true;
			if (optarg)
